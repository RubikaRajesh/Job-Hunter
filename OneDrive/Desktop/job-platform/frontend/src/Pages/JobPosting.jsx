...
function JobPosting() {
  ...
  // Create a copy without description (since we'll generate it)
    ...
    if (Array.isArray(value)) {
      if (value.length === 0) {
        setDialog({
          ...
          message: `Please add at least one item in ${field}.`,
          buttonText: "OK",
        });
        return;
      }
    } else if (field === 'salaryRange') {
      // Special check for salaryRange object
      if (!value?.from || !value?.to || value.from === 0 || value.to === 0) {
        setDialog({
          isOpen: true,
          title: "Incomplete Form",
          message: "Please fill both salary range fields.",
          buttonText: "OK",
        });
        return;
      }
  ...
  // Optional: check that skills array is not empty (already handled by skills being an array)
  ...
  try {
    ...
    setFormData({ ...formData, description: res.data });
  } catch (error) {
