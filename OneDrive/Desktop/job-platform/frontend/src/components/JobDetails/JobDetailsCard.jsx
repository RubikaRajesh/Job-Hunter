...
function JobDetailsCard({ jobData }) {
  ...
  const datePostedObj = datePosted ? new Date(datePosted) : new Date();
  const now = new Date();
  const diffTime = Math.abs(now - datePostedObj);
  ...
  const saveJob = async () => {
    ...
    try {
      ...
      setDialog({
        ...
        message: "The job has been saved successfully. You can view it in your saved jobs.",
        buttonText: "Got it!",
      });
    } catch (error) {
      if (error.response?.data?.message === "Job is already saved") {
        setDialog({
          ...
          message: "You have already saved this job. Please check your saved jobs.",
          buttonText: "Okay",
        });
      } else {
        setDialog({
  ...
  const applyForJob = async () => {
    ...
    try {
      ...
      setDialog({
        ...
        message: "Your application has been submitted successfully. Your profile has been shared with the recruiter.",
        buttonText: "Got it!",
      });
    } catch (error) {
      if (error.response?.data?.message === "Job has already been applied for") {
        setDialog({
          ...
          message: "You have already applied for this job. Your profile has been shared with the recruiter.",
          buttonText: "Okay",
        });
      } else {
        setDialog({
  ...
  const isSeeker = userData?.role === "seeker"; // Note: backend uses 'seeker', not 'jobSeeker'
    ...
    <div className="flex flex-col gap-6 border p-4 rounded-3xl shadow">
      <div className="flex justify-between border-b pb-5">
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-sm text-gray-600 font-medium">
              {employer?.companyName || employer?.userProfile?.companyName || "Company Name"}
            </p>
          </div>
        ...
        <div>
          <div className="h-20 w-20 rounded-3xl border overflow-hidden flex justify-center items-center">
            <img 
              src={employer?.companyLogo || employer?.userProfile?.companyLogo || "/default-company-logo.png"} 
              alt="Company Logo"
            />
          </div>
        </div>
      ...
      <div className="flex justify-between items-center text-sm">
        <div className="flex gap-3">
          ...
          <div className="font-light">
            Applicants: <span className="font-medium">{numberOfApplicants}</span>
          </div>
        </div>
        <div className="flex gap-5">
          <button
