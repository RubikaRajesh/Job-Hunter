...
function CompanyCard({ bgColor, company }) {
  ...
  const companyLogo = company?.companyLogo || 'https://via.placeholder.com/80';
  const companyDescription = company?.companyDescription || '';
  const location = company?.location || '';

  // Safe defaults for missing fields
  const companySize = company?.companySize || { from: 0, to: 0 };
  const companySocialProfiles = company?.companySocialProfiles || {};
  const jobListings = company?.jobListings || [];
