...
import { setUser } from "../../store/authSlice"; // make sure this action exists
...
function Login() {
  ...
  const makeLoginRequest = async (userData) => {
    setLoading(true);
    try {
      // Login – this sets the cookie and returns user data (if your backend does)
      await userService.login(userData);

      // Fetch current user to get full profile
      ...
      if (currentUser) {
        ...
        if (currentUser.role === "seeker") {
          navigate("/profile"); // or wherever you want to send seekers
